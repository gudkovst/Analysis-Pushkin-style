...
def get_data(periods: list[period_type], features: list[TextFeature], labels: list[int] = None, train_part: float = 0.8,
             val_part: float = 0.1,
             not_null_bord: int = 0, is_shuffle: bool = True) -> Data:
    fs = list()
    lens_train = list()
    lens_val = list()
    labels = labels or [i for i in range(len(periods))]
    for period in periods:
        f = period2features(period, features, not_null_bord)
        if is_shuffle:
            shuffle(f)
        fs.append(f)
